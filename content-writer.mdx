...
1. In the right panel, open **Publishing**.
2. Select the post **Status**.
3. Click the settings icon next to **Status**.
4. Select the **Post Type**.
5. Select the **Author**.
6. Select categories if the post type supports them.
7. Enable **Table of Contents** if you want a generated table of contents added to the top of the post.
8. Enable **Optimize URL** if you want AI Puffer to generate a cleaner WordPress slug.
...
1. In the right panel, open **Images**.
...
6. Set the image count, placement, WordPress image size, and alignment.
7. Generate one item and review the image previews before saving the post.
...
4. Sync models.
5. In Content Writer, open **Images**.
6. Select an image mode.
7. Select an OpenAI image model from **Image source**.
8. Open **Prompts** if you want to change the image prompt or featured image prompt.
...
8. Edit the image prompts if needed.
...
4. In Content Writer, open **Images**.
...
9. Generate one item and review the selected photo.
...
9. Generate one item and review the selected image.
...
The Knowledge Base setting appears as **Context** inside Content Writer. Use it when the generated content should pull facts from indexed site data, uploaded files, or other knowledge sources you already added in **AI Puffer > Knowledge Base**.

To use Knowledge Base context:

1. Add content to **AI Puffer > Knowledge Base** first.
2. In Content Writer, open **Knowledge Base**.
...
7. For Pinecone or Qdrant, select the same embedding provider and model used when the content was added.
8. Generate one item and check whether the output used the right context.

> Screenshot placeholder: Context selector showing OpenAI, Pinecone, and Qdrant options.

| Context provider | Setup |
| --- | --- |
| **OpenAI** | Select one or more OpenAI vector stores. When the Content Writer model is OpenAI, AI Puffer can use OpenAI file search. With other AI providers, AI Puffer queries the selected store and adds matching text to the prompt. |
| **Pinecone** | Select a Pinecone index, embedding provider, and embedding model. |
| **Qdrant** | Select a Qdrant collection, embedding provider, and embedding model. |

Context settings:

| Setting | What it does |
| --- | --- |
| **Results Limit** | How many matching pieces of knowledge AI Puffer can use. |
| **Confidence Threshold** | How close a match must be before AI Puffer includes it. |

Use a lower threshold if useful context is being missed. Use a higher threshold if unrelated context is being included.

<Info>
For Pinecone and Qdrant, the embedding model must match the data already stored in the index or collection. If the model dimensions do not match, search will not work correctly.
</Info>
